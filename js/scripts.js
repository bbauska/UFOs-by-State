...
$(document).ready(function(e) {
  $(".showNRchartTable").click(function(event){
    ...
    } else {  <!--  if( $(tableToShow).css("display") == "none") -->
...
var setupLegend = function(map_options, values_array) {
  ...
  $("#key1").text(signs[0] + addCommas(Number(map_options.rangeHigh).toFixed(dec)) 
    + signs[1] + " and above");
  $("#key2").text(signs[0] + addCommas(Number(map_options.rangeMidHigh).toFixed(dec)) 
    + signs[1] + " to " + signs[0] + modDifference(map_options.rangeHigh) + signs[1]);
  $("#key3").text(signs[0] + addCommas(Number(map_options.rangeMidLow).toFixed(dec)) 
...
// function that replaces current statevalue with one of its extra_vals, 
// depending on an index passed
...
var loadMapData = function(map_options) {
  ...
  $("#tableDiv tbody tr").each(function(index, element) {
    ...
    mapDataObj[thisState].extra_vals = extra_values.makeExtraValues(map_options, 
	  mapDataObj, thisRow);
  });
  ...
  /// take selected values from each state object and place in 
  // coresponding svg state paths
  $.each(mapDataObj, function() {
    $("circle[loc_name='"+this.stateName+"'], path[loc_name='"+this.stateName+"']", 
	$(mapID)).attr({
